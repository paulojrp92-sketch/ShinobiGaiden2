...
export interface Contact {attacker:Fighter;victim:Fighter;attack:AttackDefinition;direction:number;sourceX:number;projectile:boolean}
...
export class CombatManager {
 ...
 if(activeHitboxes(attacker).some(h=>victim.hurtboxes.some(b=>overlaps(h,b)))){a.hit.add(victim.id);contacts.push({attacker,victim,attack:a.definition,direction:attacker.facing,sourceX:attacker.x,projectile:false});}}
 ...
 const damage=Math.round(d.damage*this.combos.scale(a)*(c.projectile?1:a.meleeMultiplier));
 v.health=Math.max(0,v.health-damage);v.stun=d.hitstun;v.current=null;v.flash=8;v.vx=d.knockbackX*c.direction;v.vy=d.knockbackY;if(v.vy<0)v.grounded=false;
 v.knockdown=!!d.knockdown;v.fsm.transition(v.health===0?"KO":"HITSTUN");a.freeze=Math.max(a.freeze,d.hitstop);v.freeze=Math.max(v.freeze,d.hitstop);this.combos.register(a,damage);
