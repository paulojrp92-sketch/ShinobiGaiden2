...
export class BattleScene extends Phaser.Scene {
 sim!:Simulation;private clock=new FixedClock();private views:FighterView[]=[];private debug!:Phaser.GameObjects.Graphics;private boxes=false;
 ...
 this.views=this.sim.fighters.map(f=>new FighterView(this,f));this.debug=this.add.graphics().setDepth(99);
 screen('<div class="hud">'+this.sim.fighters.map(f=>'<section><p class="eyebrow">PLAYER '+(f.id+1)+'</p><h3>'+f.definition.name+'</h3><div class="bar health"><i></i></div><div class="bar chakra"><i></i></div></section>').join('<div class="timer">99<small>ROUND 1</small></div>')+'</div><div class="battle-footer"><span>WASD MOVE · W JUMP · S + W DROP · F3 BOXES</span>'+button("home","HOME")+'</div>');
 ...
 this.clock.advance(delta,()=>{const frames=input.poll();if(selection.mode!=="versus")frames[1]=neutralInput();this.sim.step(frames);});
 this.views.forEach(v=>v.update(this.sim.frame));this.debug.clear();if(this.boxes)for(const f of this.sim.fighters){const b=f.body;this.debug.lineStyle(2,0x55aaff).strokeRect(b.x,b.y,b.width,b.height);for(const h of f.hurtboxes)this.debug.lineStyle(1,0x44ff88).strokeRect(h.x,h.y,h.width,h.height);}
